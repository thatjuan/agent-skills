#!/usr/bin/env node
// ============================================================================
// build-assets.mjs — Jaunt app asset pipeline
// ============================================================================
...
const CONFIG = {
  ...
  logo: 'logo-jaunt-outlined.svg',
  out: 'dist/assets',
  brand: {
    name: 'Jaunt',
    shortName: 'Jaunt',
    description: 'Jaunt — a short, good trip around the neighborhood.',
    themeColor: '#E4543C',
...
async function main() {
  ...
  // Stage 4: iOS AppIcon.appiconset
  console.log('[4/6] Building iOS AppIcon.appiconset...');

  // Light variant: flattened on brand background (Signal Cream)
  await sharp(iconPath, { density: 384 })
  ...
  // Dark variant: transparent alpha preserved
  await sharp(iconPath, { density: 384 })
    .resize(1024, 1024, { fit: 'contain', background: { r: 0, g: 0, b: 0, alpha: 0 } })
    .png()
    .toFile(join(out, 'AppIcon.appiconset', 'icon-dark-1024.png'));

  // Tinted variant: grayscale, transparent
  await sharp(iconPath, { density: 384 })
    .resize(1024, 1024, { fit: 'contain', background: { r: 0, g: 0, b: 0, alpha: 0 } })
    .grayscale()
    .png()
  ...
  const iosContents = {
    images: [
      { filename: 'icon-light-1024.png', idiom: 'universal', platform: 'ios', size: '1024x1024' },
  ...
  for (const { name, w, h } of socialSizes) {
    const inner = Math.round(Math.min(w, h) * 0.55);
    const iconBuf = await sharp(iconPath, { density: 384 })
      .resize(inner, inner, { fit: 'contain', background: { r: 0, g: 0, b: 0, alpha: 0 } })
      .png()
      .toBuffer();

    // Signal Cream background, center the icon
    await sharp({
      create: {
  ...
  // Stage 6: PNG optimization (optional)
