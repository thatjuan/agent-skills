...
> Delegate code implementation to the OpenAI Codex CLI (gpt-5.5) via non-interactive `codex exec` — with a strict prompt contract, sandboxing, and mandatory diff verification afterward.
...
- *"Ship this migration with gpt-5.5, it's mechanical."*
- Model-routing policies that send bulk/clear-spec implementation to gpt-5.5
...
2. Builds the invocation: `codex -a never exec -m gpt-5.5 -c model_reasoning_effort='"high"' -s workspace-write --output-last-message /tmp/codex-final.txt "…" </dev/null`.
